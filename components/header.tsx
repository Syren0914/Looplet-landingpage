              ...
              {navLinks.map((link) => (
                link.dropdown ? (
                  <div key={link.label} className="relative" ref={featuresDropdownRef}>
                    <button onClick={() => setIsFeaturesOpen(!isFeaturesOpen)} className="flex items-center gap-1 text-sm font-medium text-white hover:text-cyan-500 transition-colors duration-300 focus:outline-none">
                      {link.label}
                  ...
                  <a key={link.label} href={link.href} className="text-sm font-medium text-white hover:text-cyan-500 transition-colors duration-300">
                    {link.label}
                  </a>
                )
              ))}
